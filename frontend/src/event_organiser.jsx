...
export default function EventOrganiser({ user, editingEvent, onEditComplete }) {
  ...
  async function loadRequests() {
    const response = await fetch(`${API}/event-organisers/${user.id}/submitted-requests`);
  ...
  async function saveChanges(event) {
    ...
    try {
      const response = await fetch(`${API}/event-organisers/${user.id}/requests/${editingId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, event_capacity: Number(form.event_capacity) }) });
  ...
  async function submitForm(event) {
    ...
    if (form.end_time <= form.start_time) return setNotice({ type: 'error', text: 'End time must be later than the start time.' });
    try {
      const response = await fetch(`${API}/event-organisers/${user.id}/requests/submit`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...form, event_capacity: Number(form.event_capacity) }) });
      const result = await response.json();
  ...
  async function submitRequest(id) {
    const response = await fetch(`${API}/event-organisers/${user.id}/requests/${id}/submit`, { method: 'POST' });
    const result = await response.json();
